...
class AllinOneDataset(DatasetTemplate):
    ...
    def once_getitem(self, index):
        ...
        if len(
            self.once_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST
        ) == 0 or np.random.random(1) > self.once_dataset_cfg.DATA_AUGMENTOR.MIX.get(
            'PROB', 0
        ):
            ...
            if len(self.once_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST) == 1:
                if self.once_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST[0] == 'mix_up':
                    data_dict = self.prepare_mixup_data(input_dict_1, input_dict_2)

                if self.once_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST[0] == 'cut_mix':
                    data_dict = self.prepare_cutmix_data(input_dict_1, input_dict_2)
            else:
                index = np.random.randint(
                    len(self.once_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST)
                )
                if (
                    self.once_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST[index]
                    == 'mix_up'
                ):
                    data_dict = self.prepare_mixup_data(input_dict_1, input_dict_2)
    ...
    def nuscenes_getitem(self, index):
        if len(
            ...
            nus_points_1 = self.nuscenes_dataset.get_lidar_with_sweeps(
                index, max_sweeps=self.nuscenes_dataset_cfg.MAX_SWEEPS
            )
            nus_points_2 = self.nuscenes_dataset.get_lidar_with_sweeps(
            ...
            if not self.nuscenes_dataset_cfg.PRED_VELOCITY:
                data_dict['gt_boxes'] = data_dict['gt_boxes'][:, [0, 1, 2, 3, 4, 5, 6]]
    ...
    def argo2_getitem(self, index):
        if len(
            ...
            if 'annos' in info_2:
                ...
                input_dict_2.update({'gt_names': gt_names, 'gt_boxes': gt_bboxes_3d})
    ...
    def waymo_getitem(self, index):
        # print("getitem waymo")
        if len(
            self.waymo_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST
        ) == 0 or np.random.random(1) > self.waymo_dataset_cfg.DATA_AUGMENTOR.MIX.get(
            'PROB', 0
        ):
            ...
            info_1 = copy.deepcopy(self.waymo_dataset.infos[index])
            idx2 = np.random.randint(len(self.waymo_dataset.infos))
            info_2 = copy.deepcopy(self.waymo_dataset.infos[idx2])

            pc_info_1 = info_1['point_cloud']
            pc_info_2 = info_2['point_cloud']

            sequence_name_1 = pc_info_1['lidar_sequence']
            sample_idx_1 = pc_info_1['sample_idx']

            sequence_name_2 = pc_info_2['lidar_sequence']
            sample_idx_2 = pc_info_2['sample_idx']

            input_dict_1 = {'sample_idx': sample_idx_1}
            input_dict_2 = {'sample_idx': sample_idx_2}

            if (
                self.waymo_dataset.use_shared_memory
                and index < self.waymo_dataset.shared_memory_file_limit
            ):
                sa_key = f'{sequence_name_1}___{sample_idx_1}'
                points_1 = SharedArray.attach(f"shm://{sa_key}").copy()
            else:
                points_1 = self.waymo_dataset.get_lidar(sequence_name_1, sample_idx_1)

            if (
                self.waymo_dataset.use_shared_memory
                and idx2 < self.waymo_dataset.shared_memory_file_limit
            ):
                sa_key = f'{sequence_name_2}___{sample_idx_2}'
                points_2 = SharedArray.attach(f"shm://{sa_key}").copy()
            else:
                points_2 = self.waymo_dataset.get_lidar(sequence_name_2, sample_idx_2)
            ...
            input_dict_1.update(
                {
                    'points': points_1,
                    'frame_id': info_1['frame_id'],
                }
            )
            input_dict_2.update(
                {
                    'points': points_2,
                    'frame_id': info_2['frame_id'],
                }
            )
            ...
            if len(self.waymo_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST) == 1:
                if self.waymo_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST[0] == 'mix_up':
                    data_dict = self.prepare_mixup_data(input_dict_1, input_dict_2)

                if self.waymo_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST[0] == 'cut_mix':
                    data_dict = self.prepare_cutmix_data(input_dict_1, input_dict_2)
            else:
                index = np.random.randint(
                    len(self.waymo_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST)
                )
                if (
                    self.waymo_dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST[index]
                    == 'mix_up'
                ):
                    data_dict = self.prepare_mixup_data(input_dict_1, input_dict_2)
            ...
            # data_dict = self.waymo_dataset.prepare_data(data_dict=input_dict)
            data_dict['metadata'] = info_1.get('metadata', info_1['frame_id'])
            data_dict.pop('num_points_in_gt', None)
    ...
    def kitti_getitem(self, index):
        ...
        #     img_shape = info_1['image']['image_shape']
        #     get_item_list = self.kitti_dataset_cfg.get('GET_ITEM_LIST', ['points'])
        ...
        # return data_dict
        # index = 4
        # if self.kitti_dataset_cfg_merge_all_iters_to_one_epoch:
