...
#             )  # [1, C, 1, N]
#             proj_scores = (
#                 proj_scores.squeeze(0).squeeze(1).transpose(0, 1).contiguous()
#             )  # [N, C]
...
#         else:
#             # print("aug")
#             idx2 = np.random.randint(len(self.once_infos))
#             once_info_1 = copy.deepcopy(self.once_infos[index])
#             once_info_2 = copy.deepcopy(self.once_infos[idx2])

#             once_points_1 = self.get_lidar(
#                 once_info_1['sequence_id'], once_info_1['frame_id']
#             )
#             once_points_2 = self.get_lidar(
...
#                 annos1 = once_info_1['annos']

#                 # for k in range(annos['name'].shape[0]):
#                 #     annos['name'][k] = (
#                 #         self.dataset_cfg.MAP_ONCE_TO_CLASS[annos['name'][k]]
#                 #         if annos['name'][k] in self.dataset_cfg.MAP_ONCE_TO_CLASS.keys()
#                 #         else annos['name'][k]
#                 #     )

#                 input_dict_1.update(
#                     {
#                         'gt_names': annos1['name'],
#                         'gt_boxes': annos1['boxes_3d'],
#                         'num_points_in_gt': annos1.get('num_points_in_gt', None),
#                     }
#                 )

#             if 'annos' in once_info_2:
#                 annos2 = once_info_2['annos']
...
#                 input_dict_2.update(
#                     {
#                         'gt_names': annos2['name'],
#                         'gt_boxes': annos2['boxes_3d'],
#                         'num_points_in_gt': annos2.get('num_points_in_gt', None),
#                     }
#                 )
#                 # TODO : refactory using attri funciton
...
#         # infos = process_single_sequence(sample_seq_list)
#         all_infos = []
#         for info in infos:
...
#         once_data_path = Path('/mnt/nas3/Data/once').resolve()
#         once_save_path = Path('/mnt/nas3/Data/once').resolve()
#         #  data_path = Path('/mnt/nas3/Data/nuScenes').resolve()
...
class ONCEDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        if (
            ...
            or len(self.dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST) == 0
            or np.random.random(1) > self.dataset_cfg.DATA_AUGMENTOR.MIX.get('PROB', 0)
        ):
            info = copy.deepcopy(self.once_infos[index])
            frame_id = info['frame_id']
            seq_id = info['sequence_id']
            points = self.get_lidar(seq_id, frame_id)
            # if self.dataset_cfg.PREPROCESSING_DATA.get('APPLY', False):
            #     point_cloud = open3d.geometry.PointCloud()
            #     point_cloud.points = open3d.utility.Vector3dVector(points[:, :3])
            #     filtered_cloud, _ = point_cloud.remove_statistical_outlier(
            #         nb_neighbors=self.dataset_cfg.PREPROCESSING_DATA.N_POINTS,
            #         std_ratio=self.dataset_cfg.PREPROCESSING_DATA.STD,
            #     )
            #     filtered_points = np.asarray(point_cloud.points)
            #     points = np.zeros((filtered_points.shape[0], 4))
            #     points[:, :3] = filtered_points
