...
def read_feather(path, columns=None) -> pd.DataFrame:
    ...
    with path.open("rb") as file_handle:
        dataframe = read_feather(file_handle, columns=columns, memory_map=True)
    return dataframe


def process_single_segment(
    segment_path, split, info_list, ts2idx, output_dir, save_bin
):
    ...
    for frame_name in frame_path_list:
        ...
        frame_info = process_and_save_frame(
            frame_path, frame_anno, ts2idx, segname, output_dir, save_bin
        )
...
def process_and_save_frame(
    ...
    if frame_anno is not None:
        ...
        annos['num_points_in_gt'] = (
            frame_anno['num_interior_pts'].to_numpy().astype(np.int32)
        )
...
class Argo2Dataset(DatasetTemplate):
    def __init__(
        ...
        if self.training and self.dataset_cfg.get('BALANCED_RESAMPLING', False):
            self.argo2_infos = self.balanced_infos_resampling(self.argo2_infos)
    ...
    def evaluation(
        ...
        dts = self.format_results(
            results, class_names, pklfile_prefix, submission_prefix
        )
...
if __name__ == '__main__':
    ...
    info_list = []
    create_argo2_infos(
        seg_path_list, seg_split_list, info_list, ts2idx, output_dir, save_bin, 0, 1
    )
...
# python -m pcdet.datasets.argo2.argo2_dataset --root_path /mnt/nas3/Data/argo2/sensor --output_dir /mnt/nas3/Data/argo2
