...
#         if (
#             self.dataset_cfg.get('SET_NAN_VELOCITY_TO_ZEROS', False)
...
class NuScenesDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        # return data_dict
        if len(self.dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST) == 0 or np.random.random(1) > self.dataset_cfg.DATA_AUGMENTOR.MIX.get(
            'PROB', 0
        ):
            info = copy.deepcopy(self.infos[index])
            points = self.get_lidar_with_sweeps(
            ...
            if 'gt_boxes' in info:
                ...
                #     info['gt_names'][k] = (
                #         self.dataset_cfg.MAP_NUSCENES_TO_CLASS[
                #             info['gt_names'][k]
                #         ]
                #         if info['gt_names'][k]
                #         in self.dataset_cfg.MAP_NUSCENES_TO_CLASS.keys()
            ...
            nus_info_1 = copy.deepcopy(self.infos[index])
            nus_info_2 = copy.deepcopy(self.infos[idx2])

            nus_points_1 = self.get_lidar_with_sweeps(
                index, max_sweeps=self.dataset_cfg.MAX_SWEEPS
            )
            nus_points_2 = self.get_lidar_with_sweeps(
                idx2, max_sweeps=self.dataset_cfg.MAX_SWEEPS
            )
            # TODO : prob for appling shift
            # if self.dataset_cfg.get('SHIFT_COOR', None):
            #     nus_points_1[:, 0:3] += np.array(
            #         self.dataset_cfg.SHIFT_COOR, dtype=np.float32
            #     )
            ...
            if len(self.dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST) == 1:
                if (
                    self.dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST[0]
                    == 'mix_up'
                ):
                ...
                if (
                    self.dataset_cfg.DATA_AUGMENTOR.MIX.NAME_LIST[index]
                    == 'mix_up'
                ):
                    data_dict = self.prepare_mixup_data(input_dict_1, input_dict_2)
