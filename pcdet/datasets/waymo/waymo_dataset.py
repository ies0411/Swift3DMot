...
#             """
#             sequence_name = sequence_name.replace('training_', '').replace(
#                 'validation_', ''
#             )
...
#             sample_idx
#             + np.arange(sequence_cfg.SAMPLE_OFFSET[0], sequence_cfg.SAMPLE_OFFSET[1]),
#             0,
#             0x7FFFFFFF,
#         )
...
#             onehot_cur = np.zeros(
#                 (points.shape[0], len(sample_idx_pre_list) + 1)
#             ).astype(points.dtype)
#             onehot_cur[:, 0] = 1
#             points = np.hstack([points, onehot_cur])
#         else:
#             points = np.hstack(
#                 [points, np.zeros((points.shape[0], 1)).astype(points.dtype)]
#             )
#         points_pre_all = []
#         num_points_pre = []
...
#             )
#             points_pre2cur = np.dot(
#                 expand_points_pre_global, np.linalg.inv(pose_cur.T)
#             )[:, :3]
#             points_pre = np.concatenate([points_pre2cur, points_pre[:, 3:]], axis=-1)
#             if sequence_cfg.get('ONEHOT_TIMESTAMP', False):
#                 onehot_vector = np.zeros(
#                     (points_pre.shape[0], len(sample_idx_pre_list) + 1)
#                 )
...
#                 pred_boxes = self.transform_prebox_to_current(
#                     pred_boxes, pose_pre, pose_cur
#                 )
#                 pred_boxes_all.append(pred_boxes)
...
#             num_frames = (
#                 sequence_cfg.SAMPLE_OFFSET[1] - sequence_cfg.SAMPLE_OFFSET[0] + 1
#             )
#             assert num_frames > 1
#             latest_center = gt_boxes[:, 0:2]
#             oldest_center = latest_center - speed * (num_frames - 1) * 0.1
#             new_center = (latest_center + oldest_center) * 0.5
#             new_length = gt_boxes[:, 3] + np.linalg.norm(
#                 latest_center - oldest_center, axis=-1
#             )
...
#                 roiaware_pool3d_utils.points_in_boxes_gpu(
#                     torch.from_numpy(points[:, 0:3]).unsqueeze(dim=0).float().cuda(),
#                     torch.from_numpy(gt_boxes_crop[:, 0:7])
#                     .unsqueeze(dim=0)
#                     .float()
#                     .cuda(),
#                 )
...
#     )
#     parser.add_argument(
#         '--update_info_only', action='store_true', default=False, help=''
#     )
#     parser.add_argument('--use_parallel', action='store_true', default=False, help='')
#     parser.add_argument(
#         '--wo_crop_gt_with_tail', action='store_true', default=False, help=''
#     )

#     args = parser.parse_args()

#     # ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
#     ROOT_DIR = Path('/mnt/nas3/Data/waymo').resolve()
#     if args.func == 'create_waymo_infos':
#         try:
#             print("create_waymo_infos")
#             print("=======")
#             print(args.cfg_file)
#             # yaml_config = yaml.safe_load(args.cfg_file)
#             # print(test)
