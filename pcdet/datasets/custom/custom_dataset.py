...
#                 # try:
#                 #     if np.shape(gt_boxes_lidar)[1] != 7:
#                 #         print(sample_idx)
#                 # except Exception as e:
#                 #     print(np.shape(gt_boxes_lidar))
#                 #     print(sample_idx)
#                 #     raise e

#                 # raise e
#                 print(np.shape(gt_boxes_lidar))
#                 # print(np.shape(gt_boxes_lidar)[1])
#                 # print(gt_boxes_lidar[:, :7])
...
#         data_path = Path('/mnt/nas3/Data/PCD/indoor_parking/custom').resolve()
...
class CustomDataset(DatasetTemplate):
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        def kitti_eval(eval_det_annos, eval_gt_annos, map_name_to_kitti):
            ...
            kitti_class_names = [map_name_to_kitti[x] for x in class_names]
            # ap_result_str, ap_dict = kitti_eval.get_official_eval_result(
            ap_result_str, ap_dict, ret = kitti_eval.get_custom_eval_result(
                gt_annos=eval_gt_annos,
                dt_annos=eval_det_annos,
                current_classes=kitti_class_names,
            )
            return ap_result_str, ap_dict, ret

        eval_det_annos = copy.deepcopy(det_annos)
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.custom_infos]

        if kwargs['eval_metric'] == 'kitti':
            ap_result_str, ap_dict, ret = kitti_eval(
                eval_det_annos, eval_gt_annos, self.map_class_to_kitti
            )
        else:
            raise NotImplementedError
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_custom_infos':
        ...
        # ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        # data_path = Path(
        #     '/mnt/nas3/Data/PCD/indoor_parking_converted/car_ped/custom'
        # ).resolve()
        data_path = Path('/mnt/nas3/Data/carla/custom').resolve()
        print("=======================")
        # data_path = Path("/mnt/nas3/Data/kitti").resolve()
