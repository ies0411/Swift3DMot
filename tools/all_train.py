...
__CONFIG_PATH__ = {
    'kitti': './cfgs/shuffle/kitti.yaml',
    'waymo': './cfgs/shuffle/waymo.yaml',
    'nuscenes': './cfgs/shuffle/nu.yaml',
}
# logger = logging.getLogger('3d-detector')
...
def main():
    ...
    cfg_dict = {}
    start_flag = True
    logger = common_utils.create_logger(None, 0)

    if args.kitti is True:
        kitti_cfg = EasyDict()
        cfg_from_yaml_file(__CONFIG_PATH__['kitti'], kitti_cfg)
        cfg_dict['kitti'] = kitti_cfg
    if args.waymo is True:
        waymo_cfg = EasyDict()
        cfg_from_yaml_file(__CONFIG_PATH__['waymo'], waymo_cfg)
        cfg_dict['waymo'] = waymo_cfg
    if args.nu is True:
        nu_cfg = EasyDict()
        cfg_from_yaml_file(__CONFIG_PATH__['nuscenes'], nu_cfg)
        cfg_dict['nu'] = nu_cfg
    ...
    for epoch in range(args.total_epoch):
        print(f'epoch : {epoch}')
        for data_type, cfg in cfg_dict.items():
            print(f'============={data_type}===============')
            # cfg.TAG = Path(cfg).stem
            ...
            if start_flag:
                ...
                args.pretrained_model = os.path.join(
                    "../output", args.output_dir, 'ckpt', 'latest_model.pth'
                )
