...
def parse_config():
    ...
    parser.add_argument('--freeze', action='store_true', default=False, help='')
    parser.add_argument(
...
def main():
    ...
    if cfg.get('TYPE') == 'ALL':
        class_names = {
            ...
            'WAYMO': cfg.CLASS_NAMES_WAYMO,
            'KITTI': cfg.CLASS_NAMES_KITTI,
            'NUSCENES': cfg.CLASS_NAMES_NUSCENES,
            'NIA': cfg.CLASS_NAMES_NIA,
            'ARGO2': cfg.CLASS_NAMES_ARGO2,
            'ONCE': cfg.CLASS_NAMES_ONCE,
            'LYFT': cfg.CLASS_NAMES_LYFT,
        }
        ...
        model = build_network(
            model_cfg=cfg.MODEL,
            num_class=len(class_names[cfg.CLASS_NAMES]),
            dataset=train_set,
        )
    ...
    if args.freeze:
        for name, param in model.named_parameters():
            # print(name)
            # param.requires_grad = False if 'roi_head' not in name else True
            if (
                'roi_head' in name
                or 'point_head' in name
                or 'dense_head' in name
                or 'backbone_2d' in name
            ):
                # or 'backbone_2d' in name:
                # print(name)
                print(name)
    ...
    # return
    # param.requires_grad = True if 'roi_head' not in name else False for name, param in model.named_parameters()
    # for parameter in some_module.parameters():
